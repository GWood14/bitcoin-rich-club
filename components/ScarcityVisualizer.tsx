...
export const ScarcityVisualizer = ({
  units,
  status,
}: {
  units: UnitRecord[];
    ...
    <div className="grid grid-cols-7 gap-1">
      {units.map((unit) => {
        ...
        return (
          <div
            key={unit.unitId}
            className={`group relative aspect-square border ${isIssued
                ? status === "ARCHIVED"
                  ? "bg-brc-rust border-brc-rust"
          ...
          >
            {isIssued && (
              <div className="absolute inset-0 opacity-0 group-hover:opacity-100 bg-black/95 z-20 flex flex-col items-center justify-center p-1 transition-opacity pointer-events-none">
                <span className="font-mono text-[6px] tracking-tighter opacity-60">
                  ID: {unit.serial}
                </span>
                <span className="font-mono text-[8px] font-bold text-brc-green">
                  ${(unit.fiatRef ?? 0).toFixed(2)}
                </span>
              </div>
            )}
          </div>
